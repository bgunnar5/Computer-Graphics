...
#include "aneurysm_OpenGL_data.h"
...
const char *phase345FragmentShader =
...
#ifdef PHASE4
	// If the data is between 1 and 4.5, LERP color between (0.25, 0.25, 1.0) and (1.0, 1.0, 1.0)
	"  float t;\n"
	"  float r;\n"
  "  float g;\n"
  "  float b;\n"
	...
	"    b = 1.0 + (t * (1.0 - 1.0));\n"
	"  }\n"
  // If the data is between 1 and 4.5, LERP color between (1.0, 1.0, 1.0) and (1.0, 0.25, 0.25)
  ...
  // Update frag_color by mixing the shading factor
	"  float shaded_r = min(1, r * shading_amount);\n"
  "  float shaded_g = min(1, g * shading_amount);\n"
  "  float shaded_b = min(1, b * shading_amount);\n"
	"  frag_color = vec4(shaded_r, shaded_g, shaded_b, 1.0);\n"
#endif
