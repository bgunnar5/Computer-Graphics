...
//      std::cout << "\t\tNew pos = " << pos[0] << ", " << pos[1] << ", " << pos[2] << std::endl;
    ...
    double getDistance(Ball &s)
    {
      ...
      double xSq = pow((pos[0] - s.pos[0]), 2);
      double ySq = pow((pos[1] - s.pos[1]), 2);
      double zSq = pow((pos[2] - s.pos[2]), 2);
      
      double distance = sqrt(xSq + ySq + zSq);
      
    	// Return the distance between these two balls
			return distance;
    }

    bool LookForCollision(Ball &s)
    {
      // If they're touching then the distance is the combination of the two radii
      double collisionDistance = radius + s.radius;
      double currDistance = getDistance(s);
    ...
    {
      // Set accident counter back to 0 for both balls
      tSinceAccident = 0;
      s.tSinceAccident = 0;
      
      // Update boolean variables to track collision
      collisionOccured = true;
      s.collisionOccured = true;
      
      // update the ball's directions
    ...
    void collideDir(double pos1[3], double dir1[3], double pos2[3], double dir2[3])
    {
      ...
      for (int i = 0; i < 3; i++) {
        numerator[i] = pos1[i] - pos2[i];
      }
      
      denominator = sqrt(pow(numerator[0], 2) + pow(numerator[1], 2) + pow(numerator[2], 2));
...
int main() 
{
  ...
  for (int i = 0; i < numBalls; i++) {
    ballList[i].initialize(i);
//    std::cout << "Initialized ID " << i << " to be at position " << ballList[i].pos[0] << ", " << ballList[i].pos[1] << ", " << ballList[i].pos[2] << " and with direction " << ballList[i].dir[0] << ", " << ballList[i].dir[1] << ", " << ballList[i].dir[2] << std::endl;
  }
  ...
  while (!glfwWindowShouldClose(window)) 
  {
    if(TICK_LIMIT != 0)
    {
      ...
      // Initialize all balls to not have collided
      for (int i = 0; i < numBalls; i++) {
        ballList[i].collisionOccured = false;
      }
      
//      std::cout << "Working on tick " << tick << std::endl;
...
//        std::cout << "\tWorking on " << i << std::endl;
          ...
          {
            if (ballList[i].LookForCollision(ballList[j])) {
